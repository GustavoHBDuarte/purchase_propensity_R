...
```{r}
precision_recall_at_k_from_last_fit_object <- function(last_fit_object, k=50){
  
  last_fit_object = last_fit_object

  k = k

  last_fit_object %>%
       unnest(.predictions)  %>%  arrange(desc(.pred_yes)) %>% # Ordenar da maior prob de yes p/ a   menor
...
```{r}
# Armazena o retorno do comando sessionInfo() em uma variável
session_info <- sessionInfo()

session_info_text <- paste(session_info, collapse = "\n")

# Salva o retorno em um arquivo
writeLines(session_info_text, "session_info.txt")
```
...
df <- readRDS("/cloud/project/purchase_propensity_R/df_cleaned.rds")
...
## serão tunados os hiperparâmetros cp (cost_complexity), tree_depth e min_n ()
### função parsnip::tune() deve ser passada junto ao hiperparâmetro
...
#hardhat::extract_parameter_set_dials(dt_model)


# Defining grid

dt_grid <- dt_model %>% extract_parameter_set_dials() %>% grid_regular(levels = 3)
...
# Selecting the best model hyperparameters
dt_best_hyperparam <- dt_tune %>% select_best("roc_auc")


# Applying the best hyperparameters to the model
dt_best_model <- finalize_model(dt_model, dt_best_hyperparam)



# Creating workflow
dt_workflow <- workflow() %>% 
  add_model(dt_best_model) %>% 
  add_recipe(reciper)
...
dt_result <- last_fit(dt_workflow, splitter) # model training
...
dt_results_conf_matrix <- dt_result %>%
                              unnest(.predictions) %>%
                              conf_mat(truth = response, estimate = .pred_class)

print('Decision Tree Confusion Matrix')
print(dt_results_conf_matrix)


# Gerando report resultados Decision Tree
dt_model_results <- dt_result %>% 
  unnest(.predictions) %>% 
  select(.pred_yes:response)


# Calculando metrics at k
dt_metrics_at_k <-precision_recall_at_k(dt_model_results)
...
## serão tunados os hiperparâmetros mtry (num de variáveis aleatórias a considerar em cada divisão), e min_n ()
### função parsnip::tune() deve ser passada junto ao hiperparâmetro
...
rf_grid <- grid_regular(mtry(range=c(3,10)),
                        min_n(range=c(100,1000)),
                        trees(range=c(100,10000)),
                        levels = 3)
...
# Selecting the best model hyperparameters
rf_best_hyperparam <- rf_tune %>% select_best("roc_auc")


# Applying the best hyperparameters to the model
rf_best_model <- finalize_model(rf_model, rf_best_hyperparam)
...
rf_result <- last_fit(rf_workflow, splitter) # model training
...
rf_results_conf_matrix <- rf_result %>%
                              unnest(.predictions) %>%
                              conf_mat(truth = response, estimate = .pred_class)

print('Random Forest Matrix')
print(rf_results_conf_matrix)


# Gerando report resultados Decision Tree
rf_model_results <- rf_result %>% 
  unnest(.predictions) %>% 
  select(.pred_yes:response)


# Calculando metrics at k
rf_metrics_at_k <-precision_recall_at_k(rf_model_results)
...
## serão tunados os hiperparâmetros loss_reduction(), tree_depth e min_n (), sample_size(), mtry(), learn_rate()
### função parsnip::tune() deve ser passada junto ao hiperparâmetro
...
# Selecting the best model hyperparameters
xgb_best_hyperparam <- xgb_tune %>% select_best("roc_auc")


# Applying the best hyperparameters to the model
xgb_best_model <- finalize_model(xgb_model, xgb_best_hyperparam)



# Creating workflow
xgb_workflow <- workflow() %>% 
  add_model(xgb_best_model) %>% 
  add_recipe(reciper)
...
xgb_result <- last_fit(xgb_workflow, splitter) # model training
...
xgb_results_conf_matrix <- xgb_result %>%
                              unnest(.predictions) %>%
                              conf_mat(truth = response, estimate = .pred_class)

print('XGBoost Confusion Matrix')
print(xgb_results_conf_matrix)


# Gerando report resultados XGBoost
xgb_model_results <- xgb_result %>% 
  unnest(.predictions) %>% 
  select(.pred_yes:response)


# Calculando metrics at k
xgb_metrics_at_k <-precision_recall_at_k(xgb_model_results)



# COLOCAR AQUI O FINE TUNING DOS DEMAIS MODELOS



# Gerando resultado modelos
tibble(model=c("Logistic Regression","Decision Tree"),
       precision_at_k=c(lr_metrics_at_k$precision_at_k,
                        dt_metrics_at_k$precision_at_k),
       recall_at_k=c(lr_metrics_at_k$recall_at_k,
                     dt_metrics_at_k$recall_at_k))
```

```{r}
gain_curve(lr_model_results, response, .pred_yes) %>% 
  autoplot()
```

```{r}
lift_curve(lr_model_results, response, .pred_yes) %>% 
  autoplot()
```

```{r}
# Saving df as rds file
base::saveRDS(lr_result, "lr_result.rds")
base::saveRDS(dt_result, "dt_result.rds")
```
